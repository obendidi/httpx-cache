...
def file_cache(
    ...
    dummy_serializer: BaseSerializer,
) -> FileCache:
    cache = FileCache(cache_dir=file_cache_dir, serializer=dummy_serializer)
...
def test_file_cache_get_cache_filepath(mock_os_makedirs: mock.MagicMock):
    cache = FileCache(cache_dir="sync-cache")
    async_cache = AsyncFileCache(cache_dir="async-cache")
...
def test_file_cache_get_not_found(
    file_cache: FileCache, mock_os_is_not_file: mock.MagicMock
):
...
def test_file_cache_get(file_cache: FileCache, mock_os_is_file: mock.MagicMock):
    with mock.patch(
        "httpx_cache.cache.file.open", mock.mock_open(read_data=b"some-data")
    ) as mocked_open:
        cached = file_cache.get(TEST_REQUEST)
...
def async_file_cache(
    ...
    cache = AsyncFileCache(cache_dir=file_cache_dir, serializer=dummy_serializer)
    assert cache.cache_dir == file_cache_dir
    mock_os_makedirs.assert_called_with(file_cache_dir, exist_ok=True)
    return cache
